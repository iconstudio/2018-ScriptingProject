...
def make_listbox(hwnd, r: int, c: int) -> Listbox:
    ...
    full_name_scroll = Scrollbar(hwnd)
    scrollbar.grid(row=r, column=c + 1, padx=0, sticky=N + S)

    listbox = Listbox(hwnd, borderwidth="0", relief="flat", activestyle='none', yscrollcommand=scrollbar.set, width = 40)
    listbox.grid(row=r, column=c, padx=0)
    scrollbar.config(command=listbox.yview)
    full_name_scroll(command=listbox.xview)
...
class xml_parser:
    ...
    def window(self, newtitle: str) -> Toplevel:
        ...
        self.frame.geometry("600x700+%d+%d" % (80, 80))
        self.frame.configure(background='#0078D7')
...
class xml_parser:
    ...
    def window(self, newtitle: str) -> Toplevel:
        ...
        self.button_quit = make_button_grid(self.frame, "종료", 13, 5, "8", "2", self.__del__).grid(sticky=W + E,
                                                                                                  pady=10)
        return self.frame
